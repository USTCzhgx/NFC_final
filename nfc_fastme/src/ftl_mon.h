...
/* Optional periodic hook; retained for existing call sites */
void FTL_MonTick(void);

/* Optional: classify slow-path reason (you already had similar) */
typedef enum {
    WPATH_FAST = 0,
    WPATH_GC,
    WPATH_LOGFULL
...
typedef struct {
    ...
    /* Precise CPU Profiling */
    u64 merge_cpu_compute_ns_sum;
    u64 merge_cpu_compute_ns_max;
    u64 merge_cpu_stall_ns_sum;
    u64 merge_cpu_stall_ns_max;

    /* HW Merge Engine Latency */
    u64 merge_hw_time_ns_sum;
    u64 merge_hw_time_ns_max;
    u64 merge_hw_time_ns_last;
    u64 merge_hw_timeout_count;
