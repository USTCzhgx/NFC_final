 ...
 *
 * Policy:
 *   - Each log block is associated with one LBN at a time.
 *   - Updated pages for one LBN are appended only to its own log block.
 *   - Merge still uses the hardware merge engine.
 *
 * Notes:
 *   - Never-written pages are still kept as erased 0xFF.
 * ============================================================ */
...
typedef struct {
    ...
    u16 log_page;   /* which page inside the associated log block */
} BestSrc_t;
...
/* Find the newest occurrence of lpn within its associated log block. */
...
/* Pick a victim associated log block (oldest generation). Returns index or -1. */
...
int ME_MergeOneLbn(u16 lbn)
{
    ...
    if (merge_wait_busy_clear_timeout(MERGE_TIMEOUT_POLL_LIMIT) != 0) {
        stall_dt += (mon_now_ns() - t_stall_start);
        g_ftl_mon.merge_hw_timeout_count++;
...
static u32 clean_log_block(int victim_idx)
{
    ...
    return (u32)ME_MergeOneLbn(vlog->assoc_lbn);
}
...
static u32 ensure_log_space_for_lbn(u16 lbn, int *did_clean)
{
...

    {
        ...
        g_ftl_mon.merge_trigger_log_full++;
#if FTL_DEBUG
        print_cstr("FTL: cleaning log block idx=");
        ...
        print_cstr(" lbn=");
        print_u32_dec((u32)Log_Table[victim].assoc_lbn);
        print_cstr("\r\n");
#endif
        if (clean_log_block(victim) == FTL_OK) {
            g_ftl_mon.gc_free_one_log_count++;
            return FTL_OK;
        }
    }
...
void FTL_Init(void)
{
    ...
    memset(MergeBufW, 0, sizeof(MergeBufW));
...
u32 FTL_Write(u32 lpn, u8 *buffer)
{
    ...
    print_cstr(" LBN ");
    print_u32_dec((u32)log->assoc_lbn);
    print_cstr(" Page ");
    print_u32_dec((u32)(log->write_ptr - 1u));
    print_cstr(" (gen=");
