 ...
 *   - Fully-associative log blocks: a single log block may mix pages
 *     from multiple LBNs.
 *   - Read path scans ALL log blocks to locate the newest version.
 *   - When log space is exhausted, reclaim by "log cleaning":
 *       pick a victim FULL log block,
 *       merge all LBNs referenced by that log into new data blocks,
 *       erase the victim log block, and reuse it.
 *
 * Correctness notes (important):
 *   - Merge selects the globally newest version for each LPN by scanning ALL logs.
 *   - After merging one LBN, we invalidate ALL log tags for that LBN across ALL logs,
 *     preventing stale log entries from re-introducing older versions into data later.
 *
 * NAND constraints:
 ...
 *   - Does NOT change policy/correctness, only avoids repeated scans.
 * ------------------------------------------------------------ */
...
typedef struct {
    ...
    u32 gen;        /* log generation for newest compare */
} BestSrc_t;
...
static u32 merge_one_lbn_global_latest(u16 lbn)
{
    ...
    /* Pure Logic: Build best source table by scanning ALL logs once */
    ...
    for (u16 page = 0; page < USER_PAGE; page++) {
        ...
        if (g_best_src[page].found) {
            ...
            u64 src = MAKE_NAND_ADDR((u16)Log_Table[li].pbn, lp, 0, 0, 0);
...
#if FTL_DEBUG
    xil_printf("FTL: cleaning full log block idx=%d pbn=%u gen=%u\r\n",
...
u32 FTL_Write(u32 lpn, u8 *buffer)
{
    ...
    lpn_set_valid(lpn);
#if FTL_DEBUG
    xil_printf("FTL_Write: LPN %u -> LogPBN %u Page %u (gen=%u)\r\n",
           (u32)lpn, (u32)log->pbn, (u32)log_page, (u32)log->gen);
#endif
    MON_WRITE_END_PATH(t0, path);
    return FTL_OK;
}
